...
import com.example.markting_test.DTOs.CompanyDTO;
...
public class CompanyController {
    ...
    public ResponseEntity<List<CompanyDTO>> getAllCompanies() {
        return ResponseEntity.status(200).body(companyService.getCompaniesDTos());
    }


    @GetMapping("/findByCompany_id/{Company_id}")
    public ResponseEntity<CompanyDTO> getCompanyById(@PathVariable Integer Company_id) {
        return ResponseEntity.status(200).body(companyService.getCompanyById(Company_id));
    }
